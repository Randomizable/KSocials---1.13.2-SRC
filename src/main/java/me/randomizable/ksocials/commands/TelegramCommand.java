...
public class TelegramCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public TelegramCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_telegram_command")) {
                if (player.hasPermission("ksocials.telegram")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("telegram_message")));
                    }
                }else{
