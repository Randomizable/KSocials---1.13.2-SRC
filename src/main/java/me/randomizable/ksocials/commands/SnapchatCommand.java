...
public class SnapchatCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public SnapchatCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_snapchat_command")) {
                if (player.hasPermission("ksocials.snapchat")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("snapchat_message")));
                    }
                }else{
