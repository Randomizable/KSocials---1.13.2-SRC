...
public class GoogleplusCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public GoogleplusCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_googleplus_command")) {
                if (player.hasPermission("ksocials.googleplus")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("googleplus_message")));
                    }
                }else{
