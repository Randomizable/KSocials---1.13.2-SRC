...
public class McmarketCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public McmarketCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_mcmarket_command")) {
                if (player.hasPermission("ksocials.mcmarket")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("mcmarket_message")));
                    }
                }else{
