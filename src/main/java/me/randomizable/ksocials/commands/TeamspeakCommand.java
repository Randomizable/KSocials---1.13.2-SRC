...
public class TeamspeakCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public TeamspeakCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_teamspeak_command")) {
                if (player.hasPermission("ksocials.teamspeak")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("teamspeak_message")));
                    }
                }else{
