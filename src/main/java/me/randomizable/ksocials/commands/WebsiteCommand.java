...
public class WebsiteCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public WebsiteCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_website_command")) {
                if (player.hasPermission("ksocials.website")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("website_message")));
                    }
                }else{
