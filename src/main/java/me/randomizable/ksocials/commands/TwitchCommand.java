...
public class TwitchCommand implements CommandExecutor, Listener {
    ...
    public TwitchCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_twitch_command")) {
                if (player.hasPermission("ksocials.twitch")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("twitch_message")));
                    }
                }else{
