...
public class TwitterCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public TwitterCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_twitter_command")) {
                if (player.hasPermission("ksocials.twitter")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("twitter_message")));
                    }
                }else{
