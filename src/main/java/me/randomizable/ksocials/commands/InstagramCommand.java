...
public class InstagramCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public InstagramCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_instagram_command")) {
                if (player.hasPermission("ksocials.instagram")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("instagram_message")));
                    }
                }else{
