...
public class FacebookCommand implements CommandExecutor, Listener {

    private KSocials plugin;

    public FacebookCommand(KSocials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (KSocials.getInstance().getConfig().getBoolean("enable_facebook_command")) {
                if (player.hasPermission("ksocials.facebook")){
                    if (!player.isDead()) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("facebook_message")));
                    }
                }else{
